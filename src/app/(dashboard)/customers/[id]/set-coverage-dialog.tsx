"use client";

import { useState } from "react";
import { useTranslations } from "next-intl";
import {
  Dialog,
  DialogTrigger,
  DialogContent,
  DialogHeader,
  DialogTitle,
  DialogDescription,
  DialogFooter,
  DialogClose,
} from "@/components/ui/dialog";
import { Button } from "@/components/ui/button";
import { Label } from "@/components/ui/label";
import {
  Select,
  SelectTrigger,
  SelectValue,
  SelectContent,
  SelectItem,
} from "@/components/ui/select";
import type { SalesRepOption } from "@/lib/services/customers";

export function SetCoverageDialog({
  customerId,
  currentCoveredById,
  currentCoveredByName,
  salesReps,
  onCoverageUpdated,
}: {
  customerId: string;
  currentCoveredById: string | null;
  currentCoveredByName: string | null;
  salesReps: SalesRepOption[];
  onCoverageUpdated: () => void;
}) {
  const t = useTranslations();
  const [open, setOpen] = useState(false);
  const [coveredById, setCoveredById] = useState(currentCoveredById ?? "none");
  const [submitting, setSubmitting] = useState(false);
  const [error, setError] = useState<string | null>(null);

  async function handleSubmit() {
    setSubmitting(true);
    setError(null);

    const { setCustomerCoverage } = await import("@/lib/actions/customers");

    const result = await setCustomerCoverage({
      customerId,
      coveredById: coveredById === "none" ? null : coveredById,
    });

    setSubmitting(false);

    if ("error" in result) {
      setError(t(result.error));
      return;
    }

    setOpen(false);
    onCoverageUpdated();
  }

  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger render={<Button variant="outline" size="sm" />}>
        {t("customers.setCoverage")}
      </DialogTrigger>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>{t("customers.setCoverage")}</DialogTitle>
          <DialogDescription>
            {currentCoveredByName
              ? `${t("customers.coveredBy")}: ${currentCoveredByName}`
              : ""}
          </DialogDescription>
        </DialogHeader>

        <div className="space-y-4 py-2">
          <div className="space-y-2">
            <Label>{t("customers.selectCoveringRep")}</Label>
            <Select value={coveredById} onValueChange={setCoveredById}>
              <SelectTrigger>
                <SelectValue>
                  {coveredById === "none"
                    ? "—"
                    : salesReps.find((r) => r.id === coveredById)?.name ?? "—"}
                </SelectValue>
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="none">{t("customers.removeCoverage")}</SelectItem>
                {salesReps.map((rep) => (
                  <SelectItem key={rep.id} value={rep.id}>
                    {rep.name}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
          </div>
        </div>

        {error && <p className="text-sm text-red-500">{error}</p>}

        <DialogFooter>
          <DialogClose render={<Button variant="outline" />}>
            {t("customers.cancel")}
          </DialogClose>
          <Button onClick={handleSubmit} disabled={submitting}>
            {submitting ? t("app.loading") : t("app.save")}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
}
