...
async function main() {
  ...
  if (!seedPassword || seedPassword === "<SET_STRONG_PASSWORD>") {
    throw new Error(
      "SEED_USERS_PASSWORD env var is required — set a real value in .env (not the placeholder)"
    );
  }
  ...
  if (seedPassword.length < 8 || [/[a-z]/, /[A-Z]/, /[0-9]/, /[^A-Za-z0-9]/].filter((r) => r.test(seedPassword)).length < 3) {
    throw new Error("SEED_USERS_PASSWORD is too weak — min 8 chars with at least 3 of: uppercase, lowercase, digits, symbols");
  }
